...
insertLogic conn logic shoudlInsertNewUpc =do
  withTransaction conn $ runExceptT $ do
      ...
      if shoudlInsertNewUpc
        then insertUpc conn logicId (effectiveFrom logic) (upc logic)
        else updateUpc conn (effectiveFrom logic) logicId (upc logic)
