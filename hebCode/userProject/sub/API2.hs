         ...
         where 
              ...
              -- else it returns the images that contains the object specified in the query parameter
               ...
               getSpecificImage id = do
                        message <- liftIO $ withResource conns $ \conn ->
                            query conn "SELECT image_details.image_id, image_label, \
                               \confidence, tag from image_details inner join imageapi on\
                                \ image_details.image_id=imageapi.image_id where image_details.image_id=?"[id]
               ...
               imageHandler imageUrl = do
                   ...
                   imgIdLabel <- liftIO $ withResource conns $ \conn -> insertLabel label conn uploadId tagOut
                   let image_id = fst $ Prelude.head imgIdLabel
                   let imageLabel = snd $ Prelude.head imgIdLabel

                   serverResponse <- liftIO $ withResource conns $ \conn -> finalResponse conn  imageUrl image_id imageLabel uploadId auth tagOut --calling userResult function
...
startApp = do
    ...
    case config of
        Right config -> do
           ...
           let conn =  API2.initializeConnection config
