...
module           ShopperAPI.Core.Config.Config where

import           Control.Monad.IO.Class       (liftIO)
import           Data.Aeson                   (decode)
import           Data.ByteString.Lazy         as BSL (readFile)
import           Data.ByteString.Lazy.Char8   as BSL8 (pack)
import           ShopperAPI.Core.Config.Types (Configuration, configFile,configVarName)
    ...
    case configJsonString' of
        Just configJsonString -> do
            ...
            case config' of
                ...
                Nothing     -> return $ Left $
                                       "Unable to parse config obtained from env var : "
                                                                           ++ envVarName
