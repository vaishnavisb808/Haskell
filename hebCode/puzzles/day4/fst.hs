...
main = do
    ...
    print $playBingo bingoCallOrder bingoSheets
...
playBingo [] _ = -1
playBingo (currentCall:remainingCalls) bingoSheets = do
    let markedSheets = strike currentCall <$> bingoSheets
    let status = isBingo <$> markedSheets
    case elemIndex True status of
      Just index -> currentCall * sumOfUnMarked (markedSheets!!index)
      Nothing    -> playBingo remainingCalls markedSheets
...
isBingo bingoSheet = horizontalBingo || verticalBingo
    where
        ...
        isBingo' sheet  = any (\x->sum x == -5) sheet
-- | Find the given number in the bingo sheet and replace it with -1 if found
strike :: Int -> [[Int]] -> [[Int]]
strike value bingoSheet = (fmap (fmap strike') bingoSheet)
    where
        strike'::Int->Int
        strike' x
          | x==value  = -1
