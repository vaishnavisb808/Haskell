...
data User' = User' {product_quantity::Int,product_price::Int} deriving (Show)
instance FromRow User' where
...
main = do
    ...
    id<- getLine
    let id' = read id ::Int
    priceData <- (query conn "select total_price from transaction_table where transaction_id=?"[id]):: IO [Only Int]

    let totalPrice = fromOnly (head priceData)
    print totalPrice

    proData <- (query conn "select product_quantity,product_price from product_table where transaction_id=?"[id])::IO[User']
    print (proData)
